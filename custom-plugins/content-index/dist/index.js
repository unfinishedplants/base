...
function generateSiteMap(cfg, idx) {
  ...
  const createURLEntry = (slug2, content) => {
    const isTag = slug2.startsWith("tags/") || slug2 === "tags";
    const isIndex = slug2 === "index" || slug2 === "/" || slug2 === "";
    const loc = isIndex ? `https://${base}/` : `https://${joinSegments(base, encodeURI(slug2))}`;
    const hasValidDate = content.date && !isTag && !isNaN(content.date.getTime());
    return `<url>
    <loc>${loc}</loc>
    ${hasValidDate ? `<lastmod>${content.date.toISOString()}</lastmod>` : ""}
  </url>`;
  };
...
function generateRSSFeed(cfg, idx, options, limit) {
  const base = (cfg.baseUrl ?? "").replace(/\/$/, "");
  const pageTitle = cfg.pageTitle ?? "Ghost in the Voronoi";
  ...
  // Filter ONLY published articles: exclude index, 404, tags/*, private/*
  const articleEntries = Array.from(idx).filter(([rawSlug, content]) => {
    const slugStr = String(rawSlug).toLowerCase();
    if (slugStr === "index" || slugStr === "" || slugStr === "404") return false;
    if (slugStr.startsWith("tags/") || slugStr === "tags") return false;
    if (slugStr.startsWith("private/")) return false;
    if (!content.title || !content.title.trim()) return false;
    if (!content.date || isNaN(content.date.getTime())) return false;
    return true;
  });

  const createURLEntry = (slug2, content) => {
    const fullUrl = `https://${joinSegments(base, encodeURI(slug2))}`;
    const desc = (content.richContent ?? content.description ?? "").trim() || content.title;
    return `<item>
  ...
  const sortedArticles = articleEntries.sort(([_1, f1], [_2, f2]) => {
...
<rss version="2.0">
    <channel>
      ...
      <generator>Quartz -- ghost.voronoi.works</generator>
      ${items}
    </channel>
  </rss>`;
}
